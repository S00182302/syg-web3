...
export class CreateActivityComponent implements OnInit {
  ...
  constructor(private svc: SYGDatabaseService, private http: HttpClient, public router: Router) { }
  ...
  onUpload() {
    ...
    this.http.post('https:bretthenning.com/svg/upload.php', uploadData, {
      reportProgress: true,
      observe: 'events'
    })
      .subscribe(event => {
  ...
  createActivity(){
    this.filename = new Date().getTime().toString() + this.selectedFile.name.substr(this.selectedFile.name.lastIndexOf("."));
  ...
  get content() { return this.activityForm.get("content"); }
